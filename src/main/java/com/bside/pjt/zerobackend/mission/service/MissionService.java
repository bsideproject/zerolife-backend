...
public class MissionService {
    ...
    @Transactional
    public MissionProgressDto findDailyMissionProgressDeprecated(final long userId) {
        ...
        MissionProgress current = missionProgressRepository.getFirstByUserIdOrderByIdDesc(userId)
            .orElseGet(() -> {
                final Mission firstMission = missionRepository.getByOrder(1)
        ...
        if (!current.isCreatedToday()) {
            final Mission nextMission = missionRepository.getByOrder(current.missionOrder() + 1)
                .orElseThrow(() -> new ServiceException(HttpStatus.BAD_REQUEST.value(), ErrorCode.E2000));
